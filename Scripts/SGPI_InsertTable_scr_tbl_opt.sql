...
INSERT INTO scr_tbl_opt 
	VALUES(24
			...
			,'Template'
			,'tpl'
			,'M�dulo que almacena los formatos del sistema'
			,1
			,-99
			,current_date
...
INSERT INTO scr_tbl_opt 
	VALUES(25
			,2
			,5
			,'App --> Colecci�n de Referencia del CIREV'
			,'app-crf'
			,'M�dulo/Aplicaci�n de la Colecci�n de Referencia del CIREV'
			,1
			,-99
			,current_date
			,btrim(btrim("left"((((date_part('hour'::text, 'now'::text::time with time zone)::text || ':'::text) || date_part('minute'::text, 'now'::text::time with time zone)) || ':'::text) || date_part('second'::text, 'now'::text::time with time zone)::text, 8), '.'::text), ' '::text)
...
INSERT INTO scr_tbl_opt 
	VALUES(26
			...
			,'app-sit'
			,'M�dulo/Aplicaci�n de los Proyectos SIT del CIREV'
			,1
			,-99
			,current_date
			,btrim(btrim("left"((((date_part('hour'::text, 'now'::text::time with time zone)::text || ':'::text) || date_part('minute'::text, 'now'::text::time with time zone)) || ':'::text) || date_part('second'::text, 'now'::text::time with time zone)::text, 8), '.'::text), ' '::text)
